...
async def test_bulk_replace_empty_to_three_blocks(client: AsyncClient):
...
async def test_bulk_replace_removes_old_blocks(client: AsyncClient):
    ctx = await _setup(client)
    ps_id = ctx["professional_store_id"]
    headers = {"Authorization": f"Bearer {ctx['prof_token']}"}

    # First replace: 3 blocks
    await client.put(
        f"{PROF_STORES_BASE}/{ps_id}/schedules",
        json={"blocks": THREE_BLOCKS},
        headers=headers,
    ...
    # Second replace: 2 different blocks
    res = await client.put(
        f"{PROF_STORES_BASE}/{ps_id}/schedules",
...
async def test_bulk_replace_with_empty_blocks_clears_all(client: AsyncClient):
    ctx = await _setup(client)
    ps_id = ctx["professional_store_id"]
    headers = {"Authorization": f"Bearer {ctx['prof_token']}"}
...
async def test_bulk_replace_overlapping_payload_rejected(client: AsyncClient):
    ctx = await _setup(client)
    ps_id = ctx["professional_store_id"]
    headers = {"Authorization": f"Bearer {ctx['prof_token']}"}
...
async def test_bulk_replace_unauthenticated(client: AsyncClient):
    ctx = await _setup(client)
    ps_id = ctx["professional_store_id"]
...
async def test_bulk_replace_wrong_owner_forbidden(client: AsyncClient):
    ctx = await _setup(client)
    ps_id = ctx["professional_store_id"]

    # Create a second professional belonging to a different store
